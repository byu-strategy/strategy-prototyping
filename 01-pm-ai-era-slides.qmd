...
`>` acts like “Save As” — it overwrites the file.
...
`>>` acts like “Add to end” — it appends text instead of overwriting.
...
We also need to install something called Node.js which is a tool that lets us run JavaScript programs, and we need it (along with `npm`, its package installer) before we can set up Claude Code or the [Gemini CLI](https://blog.google/technology/developers/introducing-gemini-cli-open-source-ai-agent/).
...
- [Claude Code from Anthropic](https://www.anthropic.com/pricing)
