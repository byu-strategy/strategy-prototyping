...
In order to be a serious AI-Powered PM or AI Product PM, you need to get comfortable interacting with your computer via the [command line](https://en.wikipedia.org/wiki/Command-line_interface).
...
# Open your new folder (directory) in Windows Explorer or Mac Finder
open my-project
...
# list all python installations. Run these lines one at a time
which -a python
which -a python3
whereis python

# Print out where the contents of the PATH variable
echo $PATH | tr ':' '\n'

# if you don't have python install install it using the command below.
brew install python
...
where python
where python3
Get-Command python* -All | Select-Object Name, Source
# Print out where the contents of the PATH variable
$env:Path -split ';'
...
We also need to install something called Node.js which is a tool that lets us run JavaScript programs, and we need it (along with `npm`, its package installer) before we can set up Claude Code.
...
Now we can install Claude Code which we will use soon. You must first [sign up for a Pro account](https://www.anthropic.com/pricing) before being able to use Claude Code. After signing up for Pro account run the following command to install Claude Code.
...
After starting Claude, turn on [Explanatory Mode](https://docs.anthropic.com/en/docs/claude-code/output-styles) within Claude which will provide "educational 'insights' in between helping you complete software engineering tasks and will help you understand implementation choices and codebase patterns."
